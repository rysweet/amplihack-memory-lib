...
class TestPyExperience:
    def test_create_experience(self):
        exp = amr.Experience("success", "context here", "outcome here", 0.9, ["tag1"])
        assert exp.experience_type == "success"
        assert exp.context == "context here"

    def test_experience_repr(self):
        exp = amr.Experience("insight", "ctx", "out", 0.8)
        assert "insight" in repr(exp)
...
class TestPyMemoryConnector:
    def test_create_connector(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                os.chdir(tmp)
                conn = amr.MemoryConnector("test-agent", "test.db")
    ...
    def test_store_and_search(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                os.chdir(tmp)
                conn = amr.MemoryConnector("test-agent", "test.db")
                exp = amr.Experience("success", "rust is fast", "good performance", 0.9, ["rust"])
                conn.store_experience(exp)
                results = conn.search("rust", limit=5)
    ...
    def test_path_traversal_rejected(self):
        with pytest.raises(Exception):
            amr.MemoryConnector("test-agent", "../../etc/passwd")


class TestPyGraphStore:
    def test_add_and_get_node(self):
        store = amr.GraphStore()
...
class TestUtilityFunctions:
    def test_extract_entities(self):
        result = amr.extract_entities("Python is a programming language", "programming")
    ...
    def test_detect_contradiction(self):
        # Returns None if no contradiction detected, or a dict if one is found
...
class TestPySecureMemoryBackend:
    def test_create_secure_backend(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                os.chdir(tmp)
                backend = amr.SecureMemoryBackend(
                    "test-agent",
                    "secure.db",
                    scope="session_only",
                )
...
class TestPyCognitiveMemory:
    def test_store_and_recall_fact(self):
        cm = amr.CognitiveMemory("test-agent")
        cm.store_fact("astronomy", "The Earth orbits the Sun", 0.9, "test")
        facts = cm.search_facts("Earth Sun", limit=5)
        assert len(facts) > 0
